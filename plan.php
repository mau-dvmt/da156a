<?php
	include "_header.php";
?>
	<div class="row">
		<div class="col-lg-8 create-submenu">
			<h2>Planering</h2>
            <p>För enkelhetens skull är alla viktiga datum och tider sammanställda här. För att undvika motsägande information är salar inte angivna - se det <a href="http://schema.mah.se/setup/jsp/Schema.jsp?startDatum=idag&intervallTyp=m&intervallAntal=6&sprak=SV&sokMedAND=true&forklaringar=true&resurser=k.DA156A-20162-TS789-">officiella schemat</a>.</p>
            <p>Labbtillfällena är i grunden fria att disponera, men i tabellen nedan anges så långt man minst bör sträva efter att komma. (Det skadar aldrig att jobba senare delar!) Assistenter finns på plats 8-12 (förmiddag) alternativt 13-17 (eftermiddag) för att ge råd och svara på frågor.</p>
            <p>Innehållet för varje tillfälle presenteras, eller kommer att presenteras, på sidorna för <a href="/lectures.php">föreläsningar</a>, <a href="/exercises.php">laborationer</a> och <a href="/assignments.php">inlämningsuppgifter</a>.</p>
            <h3>1. Tillfällen och deadline</h3>
            <table class="table" id="plan">
                <tr class="odd header">
                    <th>Vecka</th>
                    <th>Datum</th>
                    <th>Tid*</th>
                    <th>Vad</th>
                </tr>
                <tr class="lecture even">
                    <td rowspan="3">35</td>
                    <td>29/8</td>
                    <td>10-12</td>
                    <td><a href="/lectures.php?nr=1">Föreläsning 1: Kursintroduktion</a></td>
                </tr>
                <tr class="lecture even">
                    <td>30/8</td>
                    <td>13-15</td>
                    <td><a href="/lectures.php?nr=2">Föreläsning 2: Introduktion till HTML</a></td>
                </tr>
                <tr class="laboration even">
                    <td>2/9</td>
                    <td>13-17</td>
                    <td><a href="/exercises.php?title=1">Laboration 1: Enkel personlig webbsida</a></td>
                </tr>

                <tr class="lecture odd">
                    <td rowspan="3">36</td>
                    <td>5/9</td>
                    <td>10-12</td>
                    <td><a href="/lectures.php?nr=3">Föreläsning 3: Vanliga och viktiga element i HTML</a></td>
                </tr>
                <tr class="lecture odd">
                    <td>6/9</td>
                    <td>13-15</td>
                    <td><a href="/lectures.php?nr=4">Föreläsning 4: Introduktion till CSS</a></td>
                </tr>
                <tr class="laboration odd">
                    <td>8/9</td>
                    <td>8-12</td>
                    <td>Laboration <a href="/exercises.php?title=2">2.a: Mer HTML-element</a> och <a href="/exercises.php?title=2b">2.b: Stilmallar</a></td>
                </tr>

                <tr class="lecture even">
                    <td rowspan="3">37</td>
                    <td>12/9</td>
                    <td>10-12</td>
                    <td><a href="/lectures.php?nr=5">Föreläsning 5: Layout med HTML och CSS</a></td>
                </tr>
                <tr class="lecture even">
                    <td>13/9</td>
                    <td>13-15</td>
                    <td><a href="/lectures.php?nr=6">Föreläsning 6: Introduktion till JavaScript</a></td>
                </tr>
                <tr class="laboration even">
                    <td>15/9</td>
                    <td>8-12</td>
                    <td><a href="/exercises.php?title=3">Laboration 3: Layout med CSS</a></td>
                </tr>

                <tr class="lecture odd">
                    <td rowspan="4">38</td>
                    <td>19/9</td>
                    <td>10-12</td>
                    <td><a href="/lectures.php?nr=7">Föreläsning 7: Loopar, if-satser, interaktiva webbsidor med JavaScript</a></td>
                </tr>
                <tr class="lecture odd">
                    <td>20/9</td>
                    <td>13-15</td>
                    <td><a href="/lectures.php?nr=8">Föreläsning 9: Tillgänglighet, tillämpningar med JavaScript</a></td>
                </tr>
                <tr class="laboration odd">
                    <td>21/9</td>
                    <td>8-12</td>
                    <td><a href="/exercises.php?title=4">Laboration 4: Interaktivitet med JavaScript</a></td>
                </tr>
                <tr class="tenta assignment odd">
                    <td>25/9</td>
                    <td></td>
                    <td><a href="/assignments.php?nr=1">Deadline: Inlämningsuppgift 1</a></td>
                </tr>

                <tr class="lecture even">
                    <td rowspan="3">39</td>
                    <td>26/9</td>
                    <td>10-12</td>
                    <td>Föreläsning 9: Media Queries och responsive design</td>
                </tr>
                <tr class="lecture even">
                    <td>27/9</td>
                    <td>13-15</td>
                    <td>Föreläsning 10: Mer om HTML 5 och CSS 3</td>
                </tr>
                <tr class="laboration even">
                    <td>29/9</td>
                    <td>8-12</td>
                    <td>Laboration 5: Media queries</td>
                </tr>

                <tr class="lecture odd">
                    <td rowspan="3">40</td>
                    <td>3/10</td>
                    <td>10-12</td>
                    <td>Föreläsning 11: Introduktion till Användbarhet</td>
                </tr>
				<tr class="lecture odd">
					<td>4/10</td>
					<td>13-15</td>
					<td>Föreläsning 12: Användbarhet med fokus på webben</td>
				</tr>
                <tr class="laboration odd">
                    <td>6/10</td>
                    <td>13-17</td>
                    <td>Laboration: Extratillfälle</td>
                </tr>

                <tr class="lecture even">
                    <td rowspan="3">41</td>
                    <td>11/10</td>
                    <td>13-15</td>
                    <td>Föreläsning 13: HTML: Extra</td>
                </tr>
                <tr class="laboration even">
                    <td>13/10</td>
                    <td>8-12</td>
                    <td>Laboration: Extratillfälle</td>
                </tr>
                <tr class="tenta assignment even">
                    <td>16/10</td>
                    <td></td>
                    <td>Deadline: Inlämningsuppgift 2</td>
                </tr>

                <tr class="lecture odd">
                    <td rowspan="1">42</td>
                    <td>18/10</td>
                    <td>13-15</td>
                    <td>Inför tentamen</td>
                </tr>

                <tr class="tenta even">
                    <td rowspan="1">43</td>
                    <td>24/10</td>
                    <td>8-12</td>
                    <td>Tentamen, ordinarie</td>
                </tr>

                 <tr class="tenta assignment odd">
                    <td rowspan="1">44</td>
                    <td>6/11</td>
                    <td></td>
                    <td>Deadline: Inlämningsuppgift 3</td>
                </tr>

                <tr class="tenta odd">
                    <td rowspan="1">50</td>
                    <td>12/12</td>
                    <td>8-12</td>
                    <td>Omtentamen 1</td>
                </tr>

                <tr class="tenta odd">
                    <td rowspan="1">7</td>
                    <td>16/2</td>
                    <td>14-18</td>
                    <td>Omtentamen 2</td>
                </tr>
            </table>

            <p>*Notera att <a href="https://sv.wikipedia.org/wiki/Akademisk_kvart">akademisk kvart</a> tillämpas, vilket betyder att undervisning kör igång 15 minuter efter utsatt tid.</p>


		</div>
		<div class="col-lg-4 submenu-area">
		</div>
	</div>

	<hr>

<?php
	include "_footer.php";
?>
